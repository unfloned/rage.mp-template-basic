...
/**
 * The PlayerCore. everything that has to do with the player has to go in here
 */
export class PlayerCore {
    ...
    get(playerMp: PlayerMp): Player {
        if(!isNullOrUndefined(this.players[playerMp.id])) return this.players[playerMp.id];
        return undefined;
    }
     ...
     * we save then all changes from his database in the database.
    ...
    async onPlayerLogin(playerMp: PlayerMp, inputUsername: string, inputPassword: string) {
        ...
        if(!isNullOrUndefined(this.players[playerMp.id])) return false; // player already logged in
    ...
    async onPlayerRegister(playerMp: PlayerMp, inputUsername: string, inputPassword: string) {
        ...
        if(!isNullOrUndefined(prePlayer)) return playerMp.outputChatBox("Username is already registered with this Server"); //player is already registered
        ...
        // save the player in our Database
        // here we use the Connection.manager
        await this.core.Connection.manager.save(player);
